...
import os
# 设置JAVA_OPTS环境变量，增加JVM堆内存为4GB
os.environ['JAVA_OPTS'] = '-Xmx4000M'
...
def run(eval_products):
    ...
    for file_name in ['dataset', 'eval_dataset']:
        data = get_from_csv(f'{file_name}.csv')
    ...
    for k, v in eval_products.items():
        destroy_all_instance(onto)
        eval_product = {k: v}
        build_disassembly_product(onto, eval_product)
        ...
        if end_state_component:
            new_data, err_dict = generate_data_in_format(eval_product, onto, end_state_component_instance, common_fastener,
                                                         match2err_dict)
            print(new_data, LOG='INFO')

            write_in_csv('new_data.csv', new_data)

            prod_num = len(eval_product)
            print(err_dict)
            err_list = [err for err in err_dict.values()]

        total_num += prod_num
        error = list(map(lambda x, y: x + y, error, err_list))

    acc = [(1 - err / total_num) * 100 for err in error]
    print(f"ACC: {acc}")
...
if __name__ == '__main__':
    ...
    gen_dataset(DATA)
    gen_dataset(ALL_DATA[eval_num], filename='eval_dataset')
